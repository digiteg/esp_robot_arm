...
class BraccioArmI2C(BraccioArm):
    ...
    def begin(self, soft_start_level = 0):
      if (self.robotArmInit):
          return
      super().begin(soft_start_level) #  Calling Braccio begin with Softstart level => 0 is disabled
      self._sendBegin()
    
    def movement (self, stepDelay = 10, vBase = 0, vShoulder = 15, vElbow = 0,vWrist_ver = 0, vWrist_rot = 0, vgripper = 10):
        super().movement(stepDelay, vBase, vShoulder, vElbow ,vWrist_ver , vWrist_rot , vgripper)
